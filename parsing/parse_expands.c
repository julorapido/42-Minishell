...
/*   Updated: 2024/10/28 16:17:11 by jsaintho         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	modify_cmd(t_expand e, t_cmd *cmd, char *s, int ix)
{
	...
	free(cmd->command);	
	cmd->command = (char *) malloc((ft_strlen(s) - (ft_strlen(e.key)) + (ft_strlen(e.value)) + 1) * sizeof(char));
	i = 0;
	k = 0;
	while (s[i])
	{
...
static void	apply_expands(t_minishell *t, int i)
{
	int		e;
	char	*s;
	char	*sub_s;
